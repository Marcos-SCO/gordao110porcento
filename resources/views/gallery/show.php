    ...
    <figure class="d-flex flex-column justify-content-center align-items-center m-auto">
        <a href="<?= $BASE ?>/<?= imgOrDefault('gallery', $data->img, $data->id) ?>" data-toggle="lightbox" data-lightbox="mygallery" data-title="<?= $data->img_title ?>">
            <figure style="max-width: 250px">
                <img src="<?= $BASE ?>/<?= imgOrDefault('gallery', $data->img, $data->id) ?>" alt="<?= $data->img ?>" title="<?= $data->img_title ?>" class="m-auto img-thumbnail">
                <figcaption style="white-space:nowrap;width: 450px;overflow:hidden;text-overflow:ellipsis;">
                    <p style="max-width:250px;color:#ff7a08;background-color:#fff;border:1px solid #dee2e6;border-radius:44px;border-top: none!important;padding-left:.3rem"><?= $data->img_title ?></p>
                </figcaption>
            </figure>
        </a>
        <?php
