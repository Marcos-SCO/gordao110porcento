...
<section class="formPageArea card card-body bg-light mt5">
    ...
    <form action="<?= $formActionUrl ?>" method="post" enctype="multipart/form-data" hx-post="<?= $formActionUrl ?>" hx-target="body" hx-swap="show:body:top">
        ...
        <div class="form-group">
            ...
            <img src="<?= $BASE ?>/<?= imgOrDefault('product_categories', $categoryImg, $categoryId) ?>" alt="<?= $categoryImg ?>" title="<?= $categoryName ?>" onerror="this.onerror=null;this.src='<?= $RESOURCES_PATH ?>/img/not_found/no_image.jpg';">
        </div>
