                ...
                <?php foreach ($users as $user) {
                    if ($user->status == 1) { ?>
                        <li class="d-flex flex-wrap list-group-item">
                            <figure class="userPic mr-2"><img src="<?= $BASE ?>/<?= imgOrDefault('users', $user->img, $user->id) ?>" class="userImg" onerror="this.onerror=null;this.src='<?=$BASE?>/public/resources/img/not_found/no_image.jpg';"></figure> id <?= $user->id ?> - <a href="<?= $BASE ?>/users/<?= ($_SESSION['adm_id'] == 1) ? 'edit' : 'show' ?>/<?= $user->id ?>" class="ml-1 mr-1"><?= $user->name ?></a> - <?= $user->email ?>
        ...
        <?php if ($inactiveNumber->inactive > 0) { ?>
            <ul class="inactiveList list-group">
                ...
                <?php foreach ($users as $user) {
                    if ($user->status == 0) { ?>
                        <li class="d-flex flex-wrap list-group-item">
                            <figure class="userPic mr-2"><img src="<?= $BASE ?>/<?= imgOrDefault('users', $user->img, $user->id) ?>" class="userImg" onerror="this.onerror=null;this.src='<?=$BASE?>/public/resources/img/not_found/no_image.jpg';">
